...
class AISettings(Base):
 __tablename__="ai_settings"; id:Mapped[int]=mapped_column(Integer,primary_key=True,default=1); enabled:Mapped[bool]=mapped_column(Boolean,default=False); model:Mapped[str]=mapped_column(String(100),default="gpt-4.1-mini"); encrypted_api_key:Mapped[str|None]=mapped_column(Text); connection_status:Mapped[str]=mapped_column(String(30),default="not_checked"); connection_error:Mapped[str|None]=mapped_column(String(500)); checked_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True)); updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True))
class AIFallbackLog(Base,UUIDMixin):
 __tablename__="ai_fallback_logs"; document_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("documents.id"),index=True); expense_id:Mapped[uuid.UUID|None]=mapped_column(ForeignKey("expenses.id"),index=True); missing_fields:Mapped[list]=mapped_column(JSONB); reason:Mapped[str]=mapped_column(Text); model:Mapped[str]=mapped_column(String(100)); success:Mapped[bool]=mapped_column(Boolean,default=False); primary_fields:Mapped[dict]=mapped_column(JSONB); ai_fields:Mapped[dict]=mapped_column(JSONB); final_fields:Mapped[dict]=mapped_column(JSONB); supplemented_fields:Mapped[list]=mapped_column(JSONB); duration_ms:Mapped[int]=mapped_column(Integer); error_code:Mapped[str|None]=mapped_column(String(100)); error_message:Mapped[str|None]=mapped_column(String(500)); usage:Mapped[dict|None]=mapped_column(JSONB); request_id:Mapped[str|None]=mapped_column(String(255)); status:Mapped[str]=mapped_column(String(30),default="new",index=True); created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),index=True); updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True))
