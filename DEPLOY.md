...
## PIN-код и вход по отпечатку

По умолчанию для входа используется PIN-код `8852285`. Для рабочей установки
обязательно задайте в `.env` постоянный случайный ключ сессии и публичный адрес:

```dotenv
LOGIN_PIN=8852285
AUTH_SECRET=замените-на-длинную-случайную-строку
AUTH_ORIGIN=https://example.ru
AUTH_RP_ID=example.ru
```

`AUTH_ORIGIN` должен в точности совпадать с origin сайта, а `AUTH_RP_ID` — с его
доменом. Биометрия WebAuthn работает только по HTTPS (кроме локальной разработки).
После первого входа по PIN-коду установленное PWA предложит зарегистрировать
отпечаток пальца или другой системный способ разблокировки устройства.
