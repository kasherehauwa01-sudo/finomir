...
export function ExpenseModal({ close, onSaved = () => undefined }: Props) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      // Идентификаторы приходят непосредственно из результата поиска по ИНН.
      // Так заполнение не зависит от того, успели ли загрузиться справочники.
  ...
  return <div className="overlay" role="dialog" aria-modal="true">
    <section className={`modal expense-modal ${mode === 'manual' && ocrReviewed && preview ? 'ocr-completion' : ''}`}>
      ...
      {mode === 'manual' && <form className="completion-form" onSubmit={submit}>
        ...
        <label>Партнер<select required value={partnerId} onChange={(event) => { const id = event.target.value; setPartnerId(id); setCounterpartyId(''); setTagIds(partnerDefaultTagIds(partners, id)); }}><option value="">Выберите партнера</option>{partners.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
