...
export function Dashboard() {
  ...
  return <>
    ...
    <details className="dashboard-filters"><summary><b>Фильтры</b><span>Нажмите, чтобы развернуть</span></summary><div className="dashboard-filter-grid">
      <MultiFilter label="Теги" selected={tagIds} options={tags} update={setTagIds}/><MultiFilter label="Магазины" selected={storeIds} options={stores} update={setStoreIds}/><MultiFilter label="Партнеры" selected={partnerIds} options={partners} update={updatePartners}/><MultiFilter label="Контрагенты" selected={counterpartyIds} options={availableCounterparties.map((item)=>({id:item.id,name:item.full_name}))} update={setCounterpartyIds}/>
      <label>Статус оплаты<select value={paymentStatus} onChange={(event)=>setPaymentStatus(event.target.value)}><option value="all">Все</option><option value="paid">Оплачено</option><option value="unpaid">Есть остаток</option></select></label><label>Сумма от<input type="number" min="0" value={amountFrom} onChange={(event)=>setAmountFrom(event.target.value)}/></label><label>Сумма до<input type="number" min="0" value={amountTo} onChange={(event)=>setAmountTo(event.target.value)}/></label><label>Счет<select value={invoiceDocument} onChange={(event)=>setInvoiceDocument(event.target.value)}><option value="all">Все</option><option value="yes">Есть</option><option value="no">Нет</option></select></label><label>Акт<select value={closingDocument} onChange={(event)=>setClosingDocument(event.target.value)}><option value="all">Все</option><option value="yes">Есть</option><option value="no">Нет</option></select></label><button type="button" className="link" onClick={reset}>Сбросить фильтры</button>
    </div></details>
