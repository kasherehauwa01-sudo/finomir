...
export function Expenses() {
  ...
  const togglePartnerFilter = (id: string) => { const next=partnerIds.includes(id)?partnerIds.filter((item)=>item!==id):[...partnerIds,id];setPartnerIds(next);setCounterpartyIds((current)=>current.filter((counterpartyId)=>counterparties.some((item)=>item.id===counterpartyId&&(!next.length||Boolean(item.partner_id&&next.includes(item.partner_id))))));setPage(1);setSelectedIds([]); };
  ...
  return <>
    ...
    {filtersOpen && <section className="toolbar-panel expense-filters"><div className="filter-grid"><label>Период<input type="text" inputMode="numeric" placeholder="ММ.ГГГГ" value={period} onChange={(event) => { setPeriod(event.target.value); setPage(1); }} /></label><label>Статус оплаты<select value={paymentStatus} onChange={(event) => { setPaymentStatus(event.target.value); setPage(1); }}><option value="all">Все</option><option value="paid">Оплачено</option><option value="unpaid">Есть остаток</option></select></label><label>Сумма от<input type="number" min="0" value={amountFrom} onChange={(event) => setAmountFrom(event.target.value)} /></label><label>Сумма до<input type="number" min="0" value={amountTo} onChange={(event) => setAmountTo(event.target.value)} /></label><label>Дата счета от<input type="date" value={invoiceDateFrom} onChange={(event) => setInvoiceDateFrom(event.target.value)} /></label><label>Дата счета до<input type="date" value={invoiceDateTo} onChange={(event) => setInvoiceDateTo(event.target.value)} /></label><label>Документ счета<select value={invoiceDocument} onChange={(event) => setInvoiceDocument(event.target.value)}><option value="all">Любой</option><option value="yes">Загружен</option><option value="no">Не загружен</option><option value="cash">Наличные</option></select></label><label>Закрывающий документ<select value={closingDocument} onChange={(event) => setClosingDocument(event.target.value)}><option value="all">Любой</option><option value="yes">Загружен</option><option value="no">Не загружен</option></select></label></div><div className="relation-field"><b>Партнеры</b><div className="relation-options">{partners.map((item) => <button type="button" className={`relation-chip ${partnerIds.includes(item.id)?'active':'inactive'}`} key={item.id} onClick={() => togglePartnerFilter(item.id)}>{item.name}</button>)}</div></div><div className="relation-field"><b>Контрагенты</b><div className="relation-options">{counterparties.filter((item) => !partnerIds.length || Boolean(item.partner_id&&partnerIds.includes(item.partner_id))).map((item) => <button type="button" className={`relation-chip ${counterpartyIds.includes(item.id)?'active':'inactive'}`} key={item.id} onClick={() => toggleFilter(counterpartyIds,item.id,setCounterpartyIds)}>{item.full_name}</button>)}</div></div><div className="relation-field"><b>Магазины</b><div className="relation-options">{stores.map((item) => <button type="button" className={`relation-chip ${storeIds.includes(item.id)?'active':'inactive'}`} key={item.id} onClick={() => toggleFilter(storeIds,item.id,setStoreIds)}>{item.name}</button>)}</div></div><div className="relation-field"><b>Теги</b><div className="relation-options">{tags.map((item) => <button type="button" className={`relation-chip ${tagIds.includes(item.id)?'active':'inactive'}`} key={item.id} onClick={() => toggleFilter(tagIds,item.id,setTagIds)}>{item.name}</button>)}</div></div><button type="button" className="link" onClick={resetFilters}>Сбросить все фильтры</button></section>}
