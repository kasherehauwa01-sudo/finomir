...
export function Expenses() {
  ...
  return <>
    ...
    {selected.size > 0 && <div className="selection-bar"><span>Выбрано: {selected.size}</span>{data?.total && showGlobalSelection(allVisibleSelected,selected.size,data.total) ? <button type="button" disabled={selectingAll} onClick={() => void toggleAllFiltered()}>{selectingAll ? 'Выбираем…' : globalSelectionLabel(data.total)}</button> : null}<button type="button" onClick={() => setTagModal(true)}>Изменить теги</button><button type="button" className="danger" disabled={deleting} onClick={() => void deleteSelected()}>{deleting ? 'Удаление…' : 'Удалить выбранные'}</button></div>}
