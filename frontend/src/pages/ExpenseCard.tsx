...
import type { Counterparty, ExpenseDetail, ExpenseInvoice, ExpensePayment, Partner, Store, StorePreset, Tag } from '../types';
...
export const selectSingleExpenseTag = (current: Tag[], tag: Tag) => current.some((item) => item.id === tag.id) ? [] : [tag];
...
export function ExpenseCard() {
  ...
  useEffect(() => {
    load();
    Promise.all([api<Partner[]>('/partners'), api<Counterparty[]>('/counterparties'), api<Store[]>('/stores'), api<Tag[]>('/tags'), api<StorePreset[]>('/store-presets')])
      .then(([partnerItems, counterpartyItems, storeItems, tagItems, presetItems]) => { setPartners(partnerItems); setCounterparties(counterpartyItems); setStores(storeItems); setTags(tagItems); setPresets(presetItems); });
  }, [expenseId]);
  ...
  function toggleTag(tag: Tag) {
    update('tags', selectSingleExpenseTag(current.tags, tag));
  }
  ...
  return <>
    ...
    <form className="expense-card" onSubmit={save}>
      ...
      <section className="relations-card"><h2>Магазины и теги</h2><div className="relation-field"><b>Магазины</b><StorePresetLinks presets={presets} disabled={!editing} onSelect={(storeIds) => update('allocations', storeIds.map((storeId) => current.allocations.find((item) => item.store_id === storeId) ?? { store_id: storeId, store: stores.find((item) => item.id === storeId)?.name ?? '', amount: '0' }))} /><div className="relation-options">{stores.map((store) => { const active = current.allocations.some((item) => item.store_id === store.id); return <button type="button" disabled={!editing} aria-pressed={active} className={`relation-chip ${active ? 'active' : 'inactive'}`} key={store.id} onClick={() => toggleStore(store)}>{store.name}</button>; })}</div></div><div className="relation-field"><b>Теги</b><div className="relation-options">{tags.map((tag) => { const active = current.tags.some((item) => item.id === tag.id); return <button type="button" disabled={!editing} aria-pressed={active} className={`relation-chip ${active ? 'active' : 'inactive'}`} key={tag.id} onClick={() => toggleTag(tag)}>{tag.name}</button>; })}</div></div><small className="relations-hint">{editing ? 'Для расхода можно выбрать только один тег. Новый тег заменит выбранный ранее.' : 'Цветом отмечены связанные с расходом значения.'}</small></section>
