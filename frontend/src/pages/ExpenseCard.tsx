...
export function ExpenseCard() {
  ...
  async function save(event?: FormEvent) {
    ...
    if(current.tags.length!==1){setMessage('Выберите один обязательный тег.');return;}
  ...
  async function saveInvoice(event: FormEvent) { event.preventDefault(); if (!invoiceEditor) return; try{await api(`/invoices/${invoiceEditor.id}`, { method: 'PUT', body: JSON.stringify({ invoice_number: invoiceEditor.invoice_number, invoice_date: invoiceEditor.invoice_date, amount: invoiceEditor.amount }) }); setInvoiceEditor(undefined); setMessage('Счет изменен.'); load();}catch(error){setMessage(error instanceof Error?error.message:'Не удалось изменить счет.');} }
  ...
  function changePartner(partnerId:string){const partner=partners.find(item=>item.id===partnerId);const tag=tags.find(item=>item.id===partner?.tag_id);update('partner_id',partnerId);update('counterparty_id','');update('tags',tag?[tag]:[]);}
