...
export const partnerUpdatePayload = (name: string, comment: string | null | undefined, tagId: string) => ({
  name,
...
export function DirectoryCard() {
  ...
  useEffect(() => {
    if (isPartner) {
      api<PartnerDetail>(`/partners/${itemId}`).then(setPartner).catch((error: Error) => setMessage(error.message));
      api<Tag[]>('/tags').then(setTags).catch((error: Error) => setMessage(error.message));
    }
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (isPartner && partner) {
      const updated = await api<Partner>(`/partners/${itemId}`, {
        method: 'PUT',
        body: JSON.stringify(partnerUpdatePayload(String(data.get('name')), partner.comment, String(data.get('tag_id') ?? ''))),
      });
