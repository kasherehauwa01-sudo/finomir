...
export function Dashboard() {
  ...
  useEffect(() => {
    api<{ total: string }>('/expenses/outstanding-total')
      .then((result) => setOutstandingTotal(Number(result.total)))
      .catch(() => setOutstandingTotal(0));
  }, [revision]);
  ...
  return <>
    <div className="page-head"><div><p className="eyebrow">{labels[period]}</p><h1>Дашборд</h1><p>Финансовая картина маркетингового отдела</p></div><div className="dashboard-actions">{outstandingTotal > 0 && <button className="debt-button" onClick={() => navigate('/expenses?payment_status=unpaid')}>Задолженность {money(outstandingTotal)}</button>}<button className="primary" onClick={() => setModal(true)}>+ Добавить расход</button></div></div>
