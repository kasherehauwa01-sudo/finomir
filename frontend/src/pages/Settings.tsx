...
type SMTPData = { host:string;port:number;security:'ssl'|'starttls'|'none';username:string;from_email:string;from_name:string;password_set:boolean;status:string;last_error?:string|null };
...
export function Settings() {
  ...
  const [smtp,setSmtp]=useState<SMTPData>(emptySMTP); const [smtpPassword,setSmtpPassword]=useState(''); const [message,setMessage]=useState(''); const [busy,setBusy]=useState(false); const [testOpen,setTestOpen]=useState(false); const [testEmail,setTestEmail]=useState(''); const [history,setHistory]=useState<Notification[]|null>(null);
  ...
  useEffect(()=>{if(tab==='smtp')api<SMTPResponse>('/settings/smtp').then(data=>setSmtp(normalizeSMTP(data))).catch((e:Error)=>setMessage(e.message));if(tab==='scenarios')api<Scenario[]>('/settings/scenarios').then(setScenarios).catch((e:Error)=>setMessage(e.message));},[tab]);
  async function saveSmtp(){
    if(!smtp.host.trim()||!smtp.from_email.trim()){setMessage('Заполните SMTP-сервер и email отправителя.');return;}
    setBusy(true);setMessage('');
    try{const saved=await api<SMTPResponse>('/settings/smtp',{method:'PUT',body:JSON.stringify(smtpPayload(smtp,smtpPassword))});setSmtp(normalizeSMTP(saved));setSmtpPassword('');setMessage('SMTP-настройки сохранены');}
  ...
  return (
    <>
      ...
      <div className="settings-tabs" role="tablist" aria-label="Разделы настроек">
        {SETTINGS_TABS.map((item) => <button key={item.id} type="button" role="tab" aria-selected={tab===item.id} className={tab===item.id?'active':''} onClick={()=>{setTab(item.id);setMessage('')}}>{item.label}</button>)}
      </div>
      ...
      {tab==='smtp'&&<section className="settings-card email-settings"><div className="settings-card__icon" aria-hidden="true">✉</div><div className="settings-card__content"><div className="settings-section-head"><div><h2>SMTP-сервер</h2><p>Параметры отправки системных уведомлений</p></div><span className={`status-badge status-badge--${smtp.status}`}>{smtp.status==='not_configured'?'Не настроено':smtp.status==='error'?'Ошибка':'Настроено'}</span></div>{smtp.last_error&&<p className="field-error">{smtp.last_error}</p>}<div className="settings-form"><label className="field-wide">SMTP сервер<input placeholder="smtp.example.ru" value={smtp.host} onChange={e=>setSmtp({...smtp,host:e.target.value})}/></label><label>Порт<input type="number" min="1" max="65535" value={smtp.port} onChange={e=>setSmtp({...smtp,port:Number(e.target.value)})}/></label><label>Тип защиты<select value={smtp.security} onChange={e=>setSmtp({...smtp,security:e.target.value as SMTPData['security']})}><option value="ssl">SSL/TLS</option><option value="starttls">STARTTLS</option><option value="none">Без шифрования</option></select></label><label>Логин<input autoComplete="username" value={smtp.username} onChange={e=>setSmtp({...smtp,username:e.target.value})}/></label><label>Пароль<input type="password" autoComplete="new-password" placeholder={smtp.password_set?'Оставьте пустым, чтобы не менять':'Введите пароль'} value={smtpPassword} onChange={e=>setSmtpPassword(e.target.value)}/></label><label>Email отправителя<input type="email" placeholder="mail@example.ru" value={smtp.from_email} onChange={e=>setSmtp({...smtp,from_email:e.target.value})}/></label><label>Имя отправителя<input value={smtp.from_name} onChange={e=>setSmtp({...smtp,from_name:e.target.value})}/></label></div><div className="settings-actions"><button className="primary" disabled={busy} onClick={()=>void saveSmtp()}>{busy?'Сохраняем…':'Сохранить настройки'}</button><button disabled={busy||smtp.status==='not_configured'} onClick={()=>setTestOpen(true)}>Отправить тест</button><button onClick={()=>api<Notification[]>('/notifications').then(setHistory)}>История уведомлений</button></div></div></section>}
      {tab==='scenarios'&&<section className="settings-card email-settings"><div className="settings-card__icon" aria-hidden="true">⚡</div><div className="settings-card__content"><div className="settings-section-head"><div><h2>Сценарии уведомлений</h2><p>Автоматические письма по событиям системы</p></div></div><div className="scenario-list">{scenarios.map(item=><button className="scenario-row" key={item.id} onClick={()=>void openScenario(item.id)}><span><b>{item.name}</b><small>{item.enabled?'Сценарий включен':'Сценарий выключен'}</small></span><span>Настроить <b aria-hidden="true">→</b></span></button>)}</div></div></section>}
